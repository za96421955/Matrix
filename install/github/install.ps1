...
if (-not (Test-Path $VersionFile)) {
    Write-Host "[INFO] 未找到本地版本文件，正在从 Github 下载..."
...
[IO.File]::WriteAllText((Join-Path $ConfigDir "server.url"), "https://raw.githubusercontent.com/za96421955/Matrix/latest/install/github/latest-version.txt", [Text.UTF8Encoding]::new($false))
...
function Get-LatestVersionInfo {
    $url = if (Test-Path (Join-Path $ConfigDir "server.url")) { (Get-Content (Join-Path $ConfigDir "server.url") -Raw).Trim() } else { "https://raw.githubusercontent.com/za96421955/Matrix/latest/install/github/latest-version.txt" }
    try {
...
switch ($Command) {
    ...
    "update" {
        ...
        if (-not $jarDownloaded) {
            ...
            $z01Ok = Download-File -Url "$releaseUrl/$($info['JAR_PART_Z01'])" -Destination $z01Path -Label "JAR 分卷 1/2"
            $zipOk = Download-File -Url "$releaseUrl/$($info['JAR_PART_ZIP'])" -Destination $zipPath -Label "JAR 分卷 2/2"
            if ($z01Ok -and $zipOk) {
                Write-Log "INFO" "分卷下载完成，开始合并..."
                $mergedOk = $false
                # 7z
                ...
                if (-not $mergedOk) { Write-Log "WARN" "JAR 合并失败，更新中止"; exit 1 }
            } else { Write-Log "WARN" "分卷下载失败，更新中止"; exit 1 }
        }
