...
public interface Prompt {
    ...
    interface Common {
        ...
        String AUTO_PATTERN = """
            ...
            ## 可用模式
            chat: 对话模式，闲聊。
            plan: 规划模式，适用于任务规划、需求澄清、信息收集。新任务或会话，则从规划开始，除非完全没有必要。
            execute: 执行模式，适用于几次工具调用即可完成、不需要复杂编排的任务。
            task-chain: 链模式，适用于线性或简单分支的任务流，通过构建任务列表逐步完成任务。
            task-graph: 图模式，适用于目标明确但路径不确定、需要动态拆解子任务、任务间存在复杂依赖关系的场景。
        ...
        /** 执行 */
        String EXECUTE = """
            调用工具、执行任务，对过程和结果进行完整总结，不要使用任何 emoji 表情
            """;
    ...
    interface CoT {
        ...
        String PLAN = """
            ...
            2. 输出下一步的执行计划
            3. 一步一步地完成任务，不要想着一次性就可以把所有任务都完成
