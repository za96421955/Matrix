...
import { User, Brain, X, Copy, Check, ChevronRight, Wrench, Lightbulb, Info, Cloud, MapPin, Clock, Timer, Code2, Puzzle, Settings, Terminal, QrCode, FileText, AlertCircle, Flag} from 'lucide-react'
...
const ToolCallSection = React.memo(({
    ...
    return (
        <div ref={sectionRef} className="rounded-xl border border-gray-200/50 dark:border-white/[0.06]/40 bg-white/80 dark:bg-[#1c1c20]/80 backdrop-blur-sm shadow-sm overflow-hidden">
            ...
            <button
                onClick={onToggle}
                className="w-full flex items-center gap-2 px-3 py-1.5 text-left transition-colors hover:bg-black/5 dark:hover:bg-white/5"
                ...
                {expanded && (
                    ...
                    >
                        <div className="px-3 py-2 space-y-0.5">
                            {/* 1. 思考过程区块 */}
                            {hasReasoning && (
                                <div className="bg-amber-50 dark:bg-amber-900/10 rounded-lg px-3 py-2 border-l-4 border-l-amber-400 dark:border-l-amber-500">
                                    ...
                                    <div className="text-[11px] text-amber-800 dark:text-amber-300 leading-relaxed whitespace-pre-wrap break-words">
                                        {reasoningContent}
                                    </div>
                                </div>
                            )}
                            ...
                            {/* 2. 回答内容区块 */}
                            {hasContent && (
                                <div className="rounded-lg px-3 py-2 bg-gray-50/70 dark:bg-[#252529]/40 text-xs leading-relaxed break-words text-gray-900 dark:text-gray-100">
                            ...
                            {toolCalls.map((tc, i) => {
                                ...
                                return (
                                    <div
                                        key={tc.id || i}
                                        className="border border-gray-200/40 dark:border-white/[0.06]/30 rounded-lg px-3 py-2 bg-white/60 dark:bg-[#1c1c20]/60 backdrop-blur-[2px]"
                                    >
                                        {/* 工具标题行 */}
                                        <div className="flex items-center gap-2 mb-1.5 pb-1.5 border-b border-gray-100/50 dark:border-white/[0.06]/20">
                                        ...
                                        {formattedArgs && formattedArgs !== '{}' && (
                                            <div className="mb-1.5">
                                                <span className="text-[10px] font-medium text-gray-600 dark:text-gray-300 uppercase tracking-wide mb-0.5 block">参数</span>
                                                <pre className="text-[11px] bg-gray-50/70 dark:bg-[#252529]/40 rounded-lg px-2 py-1.5 overflow-x-auto font-mono text-gray-800 dark:text-gray-200 leading-relaxed border border-gray-100/40 dark:border-white/[0.06]/20">
                                                    {formattedArgs}
                                                </pre>
                                            </div>
                                        )}
                                        ...
                                        {/* 结果 */}
                                        <div>
                                            <span className="text-[10px] font-medium text-gray-600 dark:text-gray-300 uppercase tracking-wide mb-0.5 block">结果</span>
                                            {resultContent ? (
                                                isQRCode ? (
                                                    <pre className="qrcode overflow-x-auto text-[11px] text-gray-800 dark:text-gray-200">
                                                        {resultContent.replace(/^\[QRCODE\]\n([\s\S]*?)\n\[\/QRCODE\]$/, '$1')}
                                                    </pre>
                                                ) : (
                                                    <div className="text-[11px] text-gray-800 dark:text-gray-200 leading-relaxed">
                                                        {markdownEnabled ? (
    ...
    return (
        <div className="mb-1.5">
            <button
                ...
                className="flex items-center gap-1.5 text-[11px] text-gray-500 dark:text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors"
            >
            ...
            {open && (
                <motion.div
                    initial={{ height: 0, opacity: 0 }}
                    animate={{ height: 'auto', opacity: 1 }}
                    className="mt-1 p-2 rounded-lg bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-white/[0.06] text-[11px] text-gray-600 dark:text-gray-300 leading-relaxed whitespace-pre-wrap overflow-auto max-h-40 break-words"
                >
                    {content}
                </motion.div>
            )}
...
function MessageBubble({ message, isStreaming, onDelete, toolResultsMap, isToolCallExpanded, onToggleToolCall }: MessageBubbleProps) {
            ...
            <motion.div initial={{opacity: 0, y: 8}} animate={{opacity: 1, y: 0}}
                ...
                <div
                    ...
                    <div className="flex items-center gap-1.5 mb-1">
                        ...
                        <span className="text-[11px] text-gray-500 dark:text-gray-400 font-medium">系统提示</span>
                    </div>
                    <p className="text-[11px] text-gray-600 dark:text-gray-300 leading-relaxed break-words">{message.content}</p>
                </div>
            </motion.div>
        )
    }
    ...
    if (isUser) {
        ...
        return (
            <motion.div
                ...
                className="flex items-start gap-2 px-4 py-1.5 group flex-row-reverse"
                role="listitem"
                aria-label="用户消息"
            >
                <div
                    className="flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shadow-sm bg-gray-300 dark:bg-gray-600"
                    aria-hidden="true">
                ...
                <div
                    ...
                    <div className="max-w-full break-words">
                        <div
                            className="rounded-2xl px-4 py-2.5 shadow-sm max-w-full relative ml-auto bg-gradient-to-br from-[#2563eb] to-[#3b82f6] text-white rounded-tr-sm shadow-md shadow-blue-500/20">
                            <p className="whitespace-pre-wrap text-xs leading-relaxed break-words">{renderUserContent(message.content)}</p>
                            <BubbleActions
                                showCopy={true}
                                showDelete={!!onDelete}
                                onCopy={() => handleCopyText(message.content || '')}
    ...
    if (isAssistant) {
        ...
        return (
            ...
            >
                <div
                    className="flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shadow-sm bg-black dark:bg-white"
                    aria-hidden="true">
                        ...
                        if (hasReasoning && !hasContent) {
                            return (
                                <div
                                    ...
                                    <div
                                        className="text-[11px] text-gray-600 dark:text-gray-300 leading-relaxed whitespace-pre-wrap break-words">
    ...
    if (message.role === 'flag') {
        ...
        return (
            ...
            >
                <div
                    className="flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center bg-indigo-100 dark:bg-indigo-900/40 ring-1 ring-indigo-300/40 dark:ring-indigo-700/40"
                    aria-hidden="true">
    ...
    if (message.role === 'error') {
        ...
        return (
            ...
            >
                <div
                    className="flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center bg-red-100 dark:bg-red-900/40 ring-1 ring-red-300/40 dark:ring-red-700/40"
                    aria-hidden="true">
                ...
                <div className="flex-1 min-w-0 max-w-[90%] xs:max-w-[85%] sm:max-w-[80%] md:max-w-[75%] lg:max-w-[65%] xl:max-w-[70%] 2xl:max-w-[65%]">
                    <div className="relative rounded-xl border-l-4 border-l-red-500 dark:border-l-red-500 border border-red-200 dark:border-red-800/60 bg-red-50 dark:bg-red-950/50 shadow-md shadow-red-200/20 dark:shadow-red-950/40">
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                <p className="text-[11px] text-red-800/80 dark:text-red-300/80 truncate">
                                    {message.content}
                                </p>
                            </div>
                            <motion.span
